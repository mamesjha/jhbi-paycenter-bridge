...
Publish all valid Zelle, RTP, and FedNow Event 800 messages — with PII fields tokenized — to a single unified GCP Pub/Sub topic (`payment-events`) with rich message attributes, enabling downstream subscribers to filter and route without deserializing the full payload.

## Functional Requirements

| ID | Requirement |
|----|-------------|
| FR-2.1 | The service SHALL publish each valid, PII-tokenized Event 800 message to the `payment-events` Pub/Sub topic with the sanitized JSON payload as the message body |
...
- [ ] Published message bodies contain tokenized values for all three PII fields — verified by inspecting a pulled message
