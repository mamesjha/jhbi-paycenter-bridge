...
| **Blocked By** | EPIC-0 (PayCenter Infrastructure Discovery) |
| **Estimate** | 2–3 sprints |

## Summary

Build the core Python service that connects to the JH PayCenter Kafka topic(s), consumes raw payment events for Zelle, RTP, and FedNow, tokenizes PII fields in-flight, and publishes sanitized events to GCP Pub/Sub.

## Goal

Enable reliable, real-time consumption of raw payment events from JH PayCenter with at-least-once delivery semantics, in-flight PII tokenization, proper error handling, and offset management.
...
| ID | Requirement |
|----|-------------|
| FR-1.1 | The service SHALL connect to the JH PayCenter Kafka cluster using SASL/SSL authentication with credentials sourced from GCP Secret Manager at startup |
...
| FR-1.3 | The service SHALL subscribe to the PayCenter Kafka topic(s) and consume raw payment events in real time |
| FR-1.4 | The service SHALL deserialize each Kafka message value and validate it against the confirmed PayCenter event schema (see EPIC-0 / STORY-0.4) |
| FR-1.5 | The service SHALL filter consumed events to include only those for `ZELLE`, `RTP`, or `FEDNOW` rails. Events from other rail types SHALL be acknowledged and discarded |
| FR-1.6 | The service SHALL tokenize PII fields (`sender.tokenValue`, `sender.displayName`, `receiver.tokenValue`, `receiver.displayName`, `memo`) before publishing to Pub/Sub. The original plain-text values must never appear in the published payload |
...
| NFR-1.3 | The service SHALL emit structured JSON logs for every consumed message, including `transactionId`, `railType`, `paymentStatus`, and processing outcome. Logs SHALL never contain plain-text PII values |
...
- [ ] A test Zelle, RTP, and FedNow raw event injected into Kafka each appear in Pub/Sub within 2 seconds
- [ ] Published Pub/Sub messages contain tokenized values for PII fields — no plain-text PII
- [ ] The same input PII value produces the same token across multiple runs (deterministic tokenization)
- [ ] An event with an unsupported rail type is consumed, acknowledged, and discarded — not published to Pub/Sub
...
| [STORY-1.3](../stories/STORY-1.3-event-deserialization.md) | PayCenter event deserialization and schema validation |
